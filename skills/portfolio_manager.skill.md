...
rule: Vision/视觉板块（002415/002236/603501）：如果股票连续3日以上调整（累计跌幅>5%）且当日尾盘出现放量拉升（最后30分钟成交量>前30分钟均值1.5倍）或北向资金逆势净流入，且板块动量信号非BEAR → PM必须输出至少1个Buy/Overweight，即使Bull预期涨幅仅0.8-1.0%。安全边界：Bear必须确认无流动性风险（日成交额>1亿）且无跌停/停牌风险，否则仍Hold。Bull信号可信度+30%
rule: Energy/储能板块（300750/300014/002074/002460/601727）: 锂价企稳+主力净流入+板块超跌 → 考虑 Buy; 锂价续跌+板块估值高位 → Hold
rule: 在满足 +1% 硬门槛（含板块例外）的前提下，不要对所有股票都给出 Hold。仔细分析后做出有区分度的判断
rule: 硬门槛（默认）: 如果预期 Day1 涨幅不够 1%（成本 0.11%），必须 Hold；例外见下条板块超跌反弹场景
...
rule: Solar/光伏板块（300751/601012/300274/603806/300763/600732/688390）：如果板块连续3日以上调整且当日出现尾盘资金介入迹象 → 即使预期涨幅在0.8-1.0%也可考虑Buy。安全边界：Bear必须确认无行业重大利空（如硅料价格暴跌>5%、产能过剩预警、政策补贴退坡、政策突变），否则Hold

## decision_rules_anti
<!-- SKILLOPT-EDITABLE -->

anti_pattern: 不要用Q1季报的毛利率、净利润增速、ROE、PE估值等长期财务数据否决一日游信号。24小时内毛利/PE不会改变股价，这些数据与24h涨跌无关
anti_pattern: 不要因为"行业产能过剩""全行业亏损"等长期行业判断否决一日游信号。一日游只关心明天资金会不会来
...
rule: 1% 是涨幅门槛（扣除印花税 0.05% + 佣金约 0.06% = 0.11% 成本后，仍有 0.89% 净利）
