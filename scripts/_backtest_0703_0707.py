...
if bull_total>0:
    ...
    # (round-11, H-scripts-3): 扣除交易成本 0.11%（买+卖单边成本）
    COST_PCT = 0.11
    pnl_sum = hit*(TARGET_GAIN_PCT/100.0 - COST_PCT/100.0) + stop*(-STOP_LOSS_PCT/100.0 - COST_PCT/100.0)
    for r in records:
        if r["result"]=="FLAT": pnl_sum += r["close_profit"]/100 - COST_PCT/100.0
