...
## 策略说明：T+1 一日游 (One-Day Swing, v2.5 "Reversal")

```
D1 开盘前 (北京时间 9:00 前)  →  15-Agent 多空辩论 →  PM 决策 Buy/Hold
                                  (此时 A股 D0 收盘完整 + 美股 D0 美东收盘完整)
D1 开盘 (9:30)                →  开盘买入 (仅 Buy/Overweight 信号)
D2 日内                       →  触及 +1% 止盈 或 -3% 止损 即平仓
D2 收盘前                     →  未触发则强制平仓
```

| 规则 | 参数 |
|:--|:--|
| 止盈退出 | D2 日内最高 ≥ 买入价+1% → 获利平仓 |
| 止损退出 | D2 日内最低 ≤ 买入价-3% → 强制平仓 |
| 默认平仓 | 未触发止盈/止损 → D2 收盘强制平仓 |
| 仓位约束 | 单票 ≤ 20%总仓；BEAR 日反转候选 cap 10% (v2.5) |
| 只做多 | 不做空、不融券，放弃下跌段 |
| 流动性过滤 | 日成交额 ≥ 1 亿，禁止 ST，回避跌停/停牌 |

> **预测时机**: 每日开盘前 (北京时间 9:00 前) 跑预测 — 此时 A 股已收盘 (15:00 后完整) 且美股已美东收盘 (北京时间凌晨 4:00 后完整)，两边数据双完整。盘中数据不完整，预测无效。
>
> **为什么设 1% 止盈**: A股印花税+佣金 ≈ 0.11%，1% 利润覆盖 9 倍成本。7/3~7/9 三轮实盘累计 +3 止盈 vs 6 止损，止盈赚太少的根本原因是信号质量而非止盈线太窄。

### v2.5 "Reversal" 反转版核心改进

7/17 回测暴露 v1.0 在 BEAR 日系统性踏空反弹行情 (22/24 STEP，准确率 8%)。v2.5 三项硬修复：

| 改进 | 机制 | 代码位置 |
|:--|:--|:--|
| **P0: BEAR 闸门弹性化** | BEAR 日反转候选保留 10% 仓位上限 (非一刀切 Hold)；STRONG_BEAR 仍强制 Hold | [graph/trading_graph.py](graph/trading_graph.py) L516-559 |
| **P1: 反转候选筛选** | 从 Reversal 分析师输出文本解析 `reversal_candidate` 标记 (Buy/Overweight 即为反转候选) | [graph/trading_graph.py](graph/trading_graph.py) L440-455 |
| **P2: Sell 反转豁免** | 超跌股 (RSI<30 / 连跌 2 日) 的 Sell 自动降级 Hold，避免对超跌股误发 Sell | [graph/trading_graph.py](graph/trading_graph.py) L536-543 |
| **代码层强制升级** | PM prompt 软约束不足时兜底：BEAR 日 + reversal_candidate + Hold + 通过 hard_filter → 自动升级 Overweight 10% 仓位 | [graph/trading_graph.py](graph/trading_graph.py) L545-559 |

> v2.5 核心信念: BEAR 日不再一刀切全 Hold，反转候选保留低仓位参与反弹，解决系统性踏空根因。强制升级走代码层而非 prompt 层，确保 PM 不会因保守惯性丢失反弹机会。
...
```bash
# 推荐: 使用 anaconda base 环境 (Python 3.13.9, 含 langchain_core + numpy cp313 匹配)
# Windows: C:\ProgramData\anaconda3\python.exe
...
| 7/17→7/21 | v1.0 15-agent | 24 | 0 | 0 | 2 | 22 | 8% ⚠️ | — |
| **合计** | | **224** | **7** | **7** | **75** | **137** | **40%** | |

> **7/17 回测根因**: BEAR 日系统性踏空反弹行情 (22/24 STEP)，5 个 Sell 信号中 4 个反向大涨。根因是 BEAR 闸门一刀切全 Hold + Bear 对超跌股误发 Sell。**v2.5 "Reversal" 反转版专项修复** (BEAR 闸门弹性化 + 反转候选强制参与 + Sell 反转豁免)。
>
